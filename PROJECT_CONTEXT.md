...
`archismandinda/tripcash` · currently **v1.33.0** (SW cache v40).
...
- **Tests**: `node --test tests/*.test.mjs` (147 tests; the `--test dir/`
  form breaks on Node 24 — keep the glob). CI runs on every push.
- **SW discipline**: bump `VERSION` in sw.js every release (currently v40);
