...
`archismandinda/tripcash` · currently **v1.34.0** (SW cache v41).
...
- **Tests**: `node --test tests/*.test.mjs` (155 tests; the `--test dir/`
  form breaks on Node 24 — keep the glob). CI runs on every push.
- **SW discipline**: bump `VERSION` in sw.js every release (currently v41);
